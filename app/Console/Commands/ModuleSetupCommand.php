...
class ModuleSetupCommand extends Command
{
    ...
    protected $description = 'Setup and activate a module, create public symlink and initialize module options';
    ...
    public function handle(): int
    {
        ...
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | 5. Modul opciók inicializálása
        |--------------------------------------------------------------------------
        */

        $this->initializeModuleOptions($module);
    ...
    /**
     * Modul opciók inicializálása
     */
    private function initializeModuleOptions(string $module): void
    {
        $moduleKey = strtolower($module);

        $initConfig = config("module.{$moduleKey}.init");

        $items = $initConfig['initialized_options'] ?? [];

        if (!is_array($items) || empty($items)) {
            $this->info("No initialized_options found for module.");
            return;
        }
        ...
        $this->info("Module options initialized. Created: {$created}, Skipped: {$skipped}");
    }
