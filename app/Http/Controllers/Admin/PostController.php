...
class PostController extends Controller
{
    ...
    public function index(Request $request, string $postType)
    {
        ...
        // Post típus + nyelv szűrés
        $query->where('post_type_name', $postType)
            ->where('lang', $request->filled('lang') ? $request->lang : cms_locale());
    ...
    {
        $langParentId = $request->get('lang_parent_id');

        // Új bejegyzés létrehozása
        $post = Post::create([
            'lang' => cms_locale(),
            'title' => 'Új '.$postType.(empty($lang) ? null : " - [{$lang}]"),
            'name' => 'uj-'.$postType,
            'user_id' => auth()->id(),
            'status' => 'draft',
            'post_type_name' => $postType,
            'content' => '',
            'lang_parent_id' => $langParentId,
        ]);

        // Cím és slug frissítése (ID után)
        $post->update([
            'title' => "{$post->title} - {$post->id}",
            'name' => Str::slug("{$post->name}-{$post->id}"),
        ]);
