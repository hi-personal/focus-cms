...
>
    ...
    <div class="flex-grow grid grid-flow-col grid-rows-2 xl:grid-rows-1 gap-3 items-center max-md:hidden">
        ...
        <x-admin.posts.post-editor-menu-item
            href="#"
            :class="$post->hasTranslations() ? '' : 'deletePostButton'"
            :disabled="$post->hasTranslations()"
            :title="$post->hasTranslations() ? 'Előbb töröld a fordításokat' : ''"
        ...
        <!-- Megnyit link -->
        <x-admin.posts.post-editor-menu-item
            href="{{ route('post.show.'.$post->lang, ['slug'=>$post->name]) }}"
            target="_blank"
