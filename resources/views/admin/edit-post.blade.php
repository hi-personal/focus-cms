            ...
            @if (empty($post->lang_parent_id))
                @foreach($locales as $locale)
                    @if ($post->hasTranslation($locale))
                        <a href="{{ route('post.edit', ['post_type' => $postType, 'post' => $post->translation($locale)->id]) }}" class="text-blue-600 hover:text-red-800" taerget="_self">[{{ $locale }}]</a>
                    @else
                        <a href="{{ route('post.create', ['post_type'=>$postType, 'lang'=>$locale, 'lang_parent_id'=>$post->id]) }}" class="text-blue-600 hover:text-red-800" taerget="_self">[+ {{ $locale }}]</a>
                    @endif
                @endforeach
            @else
                <a href="{{ route('post.edit', ['post_type' => $postType, 'post' => $post->lang_parent_id]) }}" target="_self">{{ config('app.locale') }}</a>
                <p>{{ $post->lang }}</p>
            @endif
        @endif
