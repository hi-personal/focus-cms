...
<x-app-layout :includeTinymce="true">
    ...
    <div class="container max-w-7xl mx-auto pb-20 px-2 md:px-4 lg:px-8 pt-1 bg-white min-h-screen">
        ...
        <h1 class="text-2xl font-bold mb-6">
            {{ __('Edit '.Str::ucfirst($post->post_type_name)) }}
        </h1>

        {{-- Breadcrumb --}}
        <x-admin.admin-breadcrumb :post="$post" />

        @if(!empty($locales) && config('app.multilang') === true)

            @if (empty($post->lang_parent_id))
                ...
                @foreach($locales as $locale)
                    ...
                    @if ($post->hasTranslation($locale))
                        ...
                        <a
                            href="{{ route('post.edit', ['post_type' => $postType, 'post' => $post->translation($locale)->id]) }}"
                    ...
                    @else
                        ...
                        <a
                            href="{{ route('post.create', ['post_type'=>$postType, 'lang'=>$locale, 'lang_parent_id'=>$post->id]) }}"
                            class="text-blue-600 hover:text-red-800"
                            target="_self"
                        >
                            [+ {{ $locale }}]
            ...
            @else
                ...
                <a
                    href="{{ route('post.edit', ['post_type' => $postType, 'post' => $post->lang_parent_id]) }}"
                    target="_self"
                >
                    {{ config('app.locale') }}
                </a>
        ...
        >
            ...
            <div
                class="inline-block mx-auto py-4 px-8 border-l-4 rounded-lg shadow-xl"
                :class="getClasses()"
                @click="show = false"
            >
        ...
        >
            ...
            <input
                type="hidden"
                name="post_type_name"
                value="{{ $post->post_type_name }}"
            >

            <input
                type="hidden"
                name="editor_status"
                value="{{ session('editorStatus') }}"
            >
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($users as $user)
                                        ...
                                        <option
                                            value="{{ $user->id }}"
                                            {{ $user->id === (old('user_id') ?? $post->user_id ?? auth()->id()) ? 'selected' : '' }}
                                        >
                                            {{ $user->name }}
                                        </option>
                            ...
                            >
                                ...
                                @foreach($allTCategories as $item)
                                    ...
                                    @php
                                        ...
                                        $defaultTitle = $item->localizedTitle(
                                            config('app.locale'),
                                            false
                                        );
            ...
            <div class="w-full h-full items-center flex bg-tranparent mx-auto p-1">
                ...
                <div class="bg-white rounded-lg shadow-xl p-3 mx-auto flex flex-col overflow-y-auto">
                    ...
                    <div class="flex justify-end">
                        ...
                        <button
                            type="button"
                            class="confirm-button py-3 px-4 bg-red-500 hover:bg-red-600 text-white border rounded-lg"
                        >
                        ...
                        <button
                            type="button"
                            class="close-button ml-2 py-3 px-4 bg-gray-500 hover:bg-gray-600 text-white border rounded-lg"
                        >
    ...
    <div
        class="hidden"
        id="post_type_name"
        data-post-type-name="{{ $post->post_type_name }}"
    ></div>
